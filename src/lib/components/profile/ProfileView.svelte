<script lang="ts">
  /**
   * ProfileView — Wrapped-lite de un usuario (mirrors UserProfileSheet de iOS).
   *
   * Vista compartida por el perfil propio (`/profile`) y el perfil público de
   * cualquier usuario (`/user/[username]`). Todo lo que muestra es no sensible
   * (avatar, "miembro desde", stats agregadas, tops) — apto para que el grupo
   * se vea entre sí, igual que la presencia "Escuchando ahora" del Sidebar.
   *
   * Layout:
   *   - Hero (mismo patrón que ArtistDetail/PlaylistDetail): avatar grande
   *     redondo + kicker + h1 username + meta-line opcional. Background derivado
   *     del color determinístico del username (mismo username = mismo color).
   *   - Period selector (semana/mes).
   *   - 4 stat cards (plays, género, BPM, energía).
   *   - Top 5 canciones + Top 5 artistas.
   *
   * Datos: /api/stats/user-stats + /api/user/<username>/preferences. Ambos
   * aceptan un username arbitrario (soft auth, no admin-gated). Shape completo
   * con totales 0 y arrays vacíos cuando no hay scrobbles.
   */
  ...
  type Props = {
    ...
    username: string;
    /** True si es el perfil del usuario logueado (cambia el kicker). */
    isSelf?: boolean;
  };
  const { username, isSelf = false }: Props = $props();
...
<div class="profile">
  ...
  <header class="hero" style:--hero-bg={heroBg}>
    ...
    <div class="hero-meta">
      <p class="kicker">{isSelf ? 'Perfil' : 'Perfil de'}</p>
  ...
  <div class="content">
    ...
    {:else if hasNoData}
      <div class="empty">
        ...
        <p>
          {isSelf
            ? 'Sin datos en este período. Reproduce algo y vuelve aquí.'
